...
def send_failure_notification(title, message):
    ...
    if time.time() - _last_notify_time.get(key, 0) >= FAILURE_NOTIFY_INTERVAL:
...
def init_logging():
    ...
    logging.info("=" * 60)
    logging.info("B站监控系统启动 (最终优化版 + 动态/评论带时间)")
    logging.info("=" * 60)
...
def safe_request(url, params, header, retries=5):
        ...
        try:
            ...
            if code in (-799, -352, -509):
                ...
                if should_log(f"ratelimit_{code}_{url}"):
                    logging.warning(f"触发风控/限流 ({code})，等待 {wait:.1f} 秒")
                time.sleep(wait)
                continue
            if code == -400:
                logging.error(f"请求参数错误: {url}")
                return data
    ...
    return {"code": -500, "message": "请求最终失败"}
...
def fetch_dynamics_page(uid, offset, header):
    params = {
        "host_mid": uid, "type": "all", "timezone_offset": "-480",
        "platform": "web", "features": "itemOpusStyle,listOnlyfans,opusBigCover,onlyfansVote,decorationCard,onlyfansAssetsV2,forwardListHidden,ugcDelete",
...
def check_new_dynamics_for_uid(uid, header, seen_dynamics, state, now_ts):
    uid_str = str(uid)
    current_state = state.setdefault(uid_str, {"last_ts": 0, "baseline": "", "offset": ""})
    ...
    if max_pub_ts > last_ts:
        current_state["last_ts"] = max_pub_ts
    if new_offset != offset:
        current_state["offset"] = new_offset
    if new_baseline:
        current_state["baseline"] = new_baseline
    has_new = False
...
def start_monitoring(header):
    ...
    push_thread.start()
    logging.info("🚀 实时推送线程已启动（最终优化版）")
    logging.info("监控服务已启动（工作时间运行）")
    while True:
        try:
            ...
            if now - last_following_refresh >= FOLLOWING_REFRESH_INTERVAL:
                ...
                if new_list:
                    ...
                    if old_set != new_set:
                        ...
                        save_following_cache(following_list)
                        save_dynamic_state(state)
                        logging.info(f"关注列表更新为 {len(following_list)} 个 UID")
            ...
            if oid and (now - last_comment_check >= COMMENT_SCAN_INTERVAL):
                ...
                if new_c:
                    ...
                    logging.info(f"💬 发送 {len(new_c)} 条新评论")
            ...
            if state_updated and (now - last_state_save > STATE_SAVE_INTERVAL):
                save_dynamic_state(state)
                last_state_save = now
            if now - last_seen_clean > SEEN_CLEAN_INTERVAL:
                clean_old_seen(seen_dynamics)
                last_seen_clean = now
            batch_index = (batch_index + 1) % max(1, (total + batch_size - 1) // batch_size)
