...
# 🌟 修复：防止复制截断，这里是完整的 DYNAMIC_BURST_DURATION
...
def init_logging():
    ...
    logging.info("=" * 60)
    logging.info("B站监控系统启动 (高隐匿反风控 + F12富文本穿透版)")
    logging.info("=" * 60)
...
def get_header():
    ...
    return {
        "Cookie": cookie,
        # 换上最真实的 Chrome 浏览器 UA，绕过 -352 拦截
...
def extract_dynamic_text(item):
    ...
    try:
        ...
        # ⚠️ 核心隔离区：只把 "真实内容" 模块丢给深层扫描器，绝不碰 module_more 等含系统词的模块！
        dyn_module = modules.get("module_dynamic") or {}

        # 针对 F12 结构 <div class="opus..."><p><span> 打造的深层收割机
                ...
                if isinstance(node, dict):
                    for k, v in node.items():
                        if isinstance(v, str):
                            ...
                            # 🎯 精准打击 F12 中的 span.text 和常规的 title/desc
