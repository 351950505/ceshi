...
def init_logging():
    ...
    logging.info("=" * 60)
    logging.info("B站监控系统启动（2026重构稳定版）")
    logging.info("=" * 60)
...
def safe_request(url, params=None, header=None, retries=3):
    for i in range(retries):
        try:
            ...
            logging.info(f"请求 {url} 状态:{r.status_code}")
            ...
            if "验证码" in txt or "风险控制" in txt:
                logging.warning("触发风控验证页，暂停300秒")
                time.sleep(300)
                continue
            ...
            if code == -352:
                logging.warning("触发352风控，暂停300秒")
                time.sleep(300)
                continue
...
def wbi_request(url, params, header):
    if time.time() - WBI["ts"] > 21600 or not WBI["img_key"]:
        update_wbi(header)

    params = sign_wbi(params)
    return safe_request(url, params=params, header=header)
